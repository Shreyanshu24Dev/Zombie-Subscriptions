...
def get_user_email(access_token: str) -> str:
    """Looks up the connected account's own address, so we know who to
    email the summary to without asking them to type it in."""
    headers = {"Authorization": f"Bearer {access_token}"}
    response = requests.get(f"{GMAIL_API_BASE}/profile", headers=headers, timeout=15)
    response.raise_for_status()
    return response.json()["emailAddress"]


def send_email(access_token: str, to_email: str, subject: str, body_text: str):
    """Sends a plain-text email as the connected user, to themselves.
    Requires the gmail.send scope granted during OAuth."""
...
def build_summary_email_body(subscriptions: list, total_annual_cost: float) -> str:
    """Plain-text summary email body -- kept simple and readable in any
    email client, no HTML needed for an MVP."""
    lines = [
        "Here's what we found scanning your subscriptions:",
        "",
    ]
    for s in subscriptions:
        lines.append(
            f"- {s['merchant']}: {s['frequency']}, "
            f"${s['avg_amount']:.2f} per charge, "
            f"~${s['estimated_annual_cost']:.2f}/year"
        )
    lines += [
        "",
        f"Estimated total: ${total_annual_cost:,.2f}/year across {len(subscriptions)} subscriptions.",
        "",
